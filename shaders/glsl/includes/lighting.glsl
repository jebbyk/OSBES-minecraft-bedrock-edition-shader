...
vec4 calculateSkyLightDiffused(vec3 normalVector, float isRain, float isDay, float isHell){
	...
	vec3 clearSkyLightColor = pow(FOG_COLOR.rgb, vec3(SKY_LIGHT_SATURATION)) * (1.0 - isRain) * 0.5; 
...
vec4 calculateMainLightDiffused(vec3 normalVector, float isDay){
	...
	vec3 sunLightColor = MAIN_LIGHT_SUN_COLOR;
	vec3 moonLightColor = MAIN_LIGHT_MOON_COLOR * 0.35;
...
vec3 calculatePointLightsDiffused(float srcPointLights){
	...
	return vec3(overalPointLightsBrightness) * POINT_LIGHTS_TINT;
}