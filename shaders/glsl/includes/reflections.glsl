...
vec4 buildRawSkySphere(vec3 relativePosition, vec3 resultLighting, float horizonOffset, float horizonScale)
{
	float horizonLine =  1.0 - (abs(relativePosition.y + horizonOffset) / length(relativePosition.xyz));
